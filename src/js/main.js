...
(function(window) {
  window['app'] = {
    ga: function(category, label) {
      window.ga('send', 'event', category, 'click', label);
      window['google_conversion_format'] = '3';
      window['google_is_call'] = true;
      var w = window;
      switch (category) {
        case 'download-mychat':
          w['google_conversion_id'] = 961777911;
          w['google_conversion_label'] = 'J1ioCMO1n1oQ96HOygM';
          w['google_conversion_value'] = 1.00;
          w['google_conversion_currency'] = 'USD';
          w['google_remarketing_only'] = false;
          break;
        case 'go-to-google-play':
          w['google_conversion_id'] = 961777911;
          w['google_conversion_label'] = 'MJTNCKK1n1oQ96HOygM';
          w['google_conversion_value'] = 1.00;
          w['google_conversion_currency'] = 'USD';
          w['google_remarketing_only'] = false;
          break;
  ...
  // utm
  var addUTM = function($elements) {
    for (var index = 0; index < $elements.length; index++) {
      var href = ($elements[index].href || '').split('?');
      ...
      for (var k in utm) {
        if (!!utm[k]) {
          referrer.push(k + '%3D' + utm[k]);
        }
      }
      data.params['referrer'] = referrer.join('%26') || 'utm_source%3Dmychat.mysquar.com%26utm_medium%3Dorganic%26utm_campaign%3Dmychat';

      var tmp = [];
      for (var key in data.params) {
        tmp.push(key + '=' + data.params[key]);
      }
      $elements[index].href = href[0] + '?' + tmp.join('&');
    }
  };
  addUTM(document.getElementsByClassName('google-play-download'));
})(window);
...
(function($) {
  ...
  $('.language-tab a').click(function() {
    $('.language-tab a.selected').removeClass('selected');
  ...
  if (vpw < 768) {
    var navigation = {
      ...
      animate: { in : 'animated fadeInUp', // Change Animation Here - Incoming Animation
